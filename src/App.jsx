...
// --- STYLES ---
const S = {
  label: { display:"block", fontSize:15, fontWeight:700, color:B.muted, marginBottom:5, textTransform:"uppercase", letterSpacing:0.8 },
  input: { width:"100%", padding:"10px 13px", borderRadius:10, border:`1.5px solid ${B.sand}`, background:"#fff", fontSize:16, color:B.dark, outline:"none", boxSizing:"border-box", fontFamily:"inherit" },
  btn: (bg=B.red,fg="#fff") => ({ background:bg, color:fg, border:"none", borderRadius:10, padding:"11px 22px", fontSize:16, fontWeight:700, cursor:"pointer", fontFamily:"inherit", transition:"opacity .15s" }),
  card: { background:"#fff", border:`1px solid ${B.sand}`, borderRadius:16, padding:"18px 20px", boxShadow:"0 2px 10px rgba(0,0,0,0.05)" },
  tag: (c) => ({ display:"inline-block", background:c+"18", color:c, fontSize:16, fontWeight:700, borderRadius:5, padding:"2px 7px", marginRight:5, textTransform:"uppercase", letterSpacing:0.5 }),
  section: { fontWeight:700, fontSize:17, color:B.dark, marginBottom:12, paddingBottom:6, borderBottom:`2px solid ${B.sand}` },
  pill: (active) => ({ background: active ? B.red : "transparent", color: active ? "#fff" : "rgba(255,255,255,0.7)", border:"none", borderRadius:"8px 8px 0 0", padding:"9px 14px", fontWeight:600, fontSize:16, cursor:"pointer", fontFamily:"inherit", whiteSpace:"nowrap", transition:"all .2s" }),
  textarea: { width:"100%", padding:"10px 13px", borderRadius:10, border:`1.5px solid ${B.sand}`, background:"#fff", fontSize:16, color:"#2C1A0E", outline:"none", boxSizing:"border-box", fontFamily:"inherit", minHeight:120, resize:"vertical", lineHeight:1.6 },
};

function Spinner({ text="Generating..." }) {
  return <div style={{ textAlign:"left", padding:"50px 0", color:B.muted, fontSize:14 }}>✨ {text}</div>;
}
function Err({ msg }) {
  return msg ? <div style={{ color:B.red, fontSize:15, marginTop:10 }}>⚠️ {msg}</div> : null;
}
...
JSON only: {"ideas":[{"title":"...","hook":"...","body":"...","cta":"...","whyItWorks":"..."}]}`);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <p style={{ color:B.muted, fontSize:16, marginBottom:20 }}>Ideas built around what's <strong>actually working</strong> on your account — activity-first, problem-led hooks.</p>
      ...
      {ideas.map((idea,i) => (
        <div key={i} style={{ ...S.card, marginTop:14 }}>
          <div style={{ display:"flex", gap:12 }}>
            <div style={{ background:B.red, color:"#fff", borderRadius:"50%", width:26, height:26, display:"flex", alignItems:"center", justifyContent:"center", fontWeight:700, fontSize:16, flexShrink:0 }}>{i+1}</div>
            <div style={{ flex:1 }}>
              <div style={{ fontWeight:700, color:B.dark, fontSize:17, marginBottom:8 }}>{idea.title}</div>
              <div style={{ marginBottom:10 }}><span style={S.tag("#E74C3C")}>Hook</span><span style={{ fontSize:15, color:B.dark }}>{idea.hook}</span></div>
              <div style={{ marginBottom:10 }}><span style={S.tag("#8E44AD")}>Body</span><span style={{ fontSize:15, color:B.dark }}>{idea.body}</span></div>
              <div style={{ marginBottom:10 }}><span style={S.tag(B.red)}>CTA</span><span style={{ fontSize:15, color:B.dark }}>{idea.cta}</span></div>
              <div style={{ background:B.softGold, borderRadius:8, padding:"7px 10px", fontSize:16, color:B.gold, marginTop:6 }}>💡 {idea.whyItWorks}</div>
            </div>
          </div>
        </div>
      ))}
...
JSON only: {"slides":[{"slide":1,"headline":"...","body":"...","visual":"...","designTip":"..."}]}`);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <p style={{ color:B.muted, fontSize:16, marginBottom:20 }}>Turn any topic into a ready-to-design carousel with visual directions for each slide.</p>
        ...
        <div style={{ marginTop:24, display:"grid", gridTemplateColumns:"repeat(auto-fill,minmax(250px,1fr))", gap:14 }}>
          {slides.map((slide,i) => (
            <div key={i} style={{ ...S.card, borderTop:`4px solid ${colors[i%colors.length]}`, padding:"16px" }}>
              <div style={{ fontSize:16, fontWeight:700, color:colors[i%colors.length], textTransform:"uppercase", letterSpacing:1, marginBottom:5 }}>Slide {slide.slide}</div>
              <div style={{ fontWeight:700, fontSize:16, color:B.dark, marginBottom:7 }}>{slide.headline}</div>
              <div style={{ fontSize:15, color:"#555", marginBottom:8, lineHeight:1.5 }}>{slide.body}</div>
              <div style={{ fontSize:16, color:B.muted, background:B.warm, borderRadius:6, padding:"6px 9px", marginBottom:6 }}>🖼️ <em>{slide.visual}</em></div>
              <div style={{ fontSize:15, color:B.gold, background:B.softGold, borderRadius:6, padding:"5px 9px" }}>✏️ {slide.designTip}</div>
            </div>
          ))}
        </div>
      )}
...
JSON only: {"repurpose":[{"platform":"...","caption":"...","hashtags":["..."],"actionTip":"..."}]}`);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <p style={{ color:B.muted, fontSize:16, marginBottom:6 }}>Film <strong>once</strong>, post everywhere. Paste your video topic and get a ready-to-use plan for all 5 platforms + Etsy.</p>
      <div style={{ background:B.softGold, borderRadius:10, padding:"10px 14px", marginBottom:20, fontSize:15, color:B.gold }}>
      ...
      {plan && plan.map((p,i) => (
        <div key={i} style={{ ...S.card, marginTop:14, borderLeft:`4px solid ${platformColors[p.platform]||B.red}` }}>
          <div style={{ fontWeight:700, color:platformColors[p.platform]||B.red, fontSize:15, marginBottom:8 }}>{p.platform}</div>
          <div style={{ fontSize:15, color:B.dark, lineHeight:1.6, marginBottom:8, background:B.cream, borderRadius:8, padding:"10px 12px" }}>{p.caption}</div>
          <div style={{ display:"flex", flexWrap:"wrap", gap:5, marginBottom:8 }}>
            {p.hashtags?.map((h,j) => <span key={j} style={{ background:B.sand, color:B.muted, fontSize:15, borderRadius:20, padding:"2px 9px" }}>{h}</span>)}
          </div>
          <div style={{ fontSize:16, color:B.green, background:B.softGreen, borderRadius:7, padding:"6px 10px" }}>✅ {p.actionTip}</div>
        </div>
      ))}
    </div>
  );
...
JSON only: {"title":"...","description":"...","keywords":["..."],"boardName":"...","etsyTip":"..."}`);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <div style={{ background:B.softRed, borderRadius:12, padding:"12px 16px", marginBottom:20, fontSize:15, color:B.red }}>
        ...
        <div style={{ marginTop:22 }}>
          {[
            ...
            <div key={key} style={{ ...S.card, marginBottom:12 }}>
              ...
              <div style={{ fontSize:15, color:"#444", lineHeight:1.6, background:B.cream, borderRadius:8, padding:"10px 12px" }}>{val}</div>
            </div>
          ))}
          <div style={{ ...S.card, marginBottom:12 }}>
            <div style={{ fontWeight:700, color:B.dark, fontSize:15, marginBottom:10 }}>🔍 Keywords to Use</div>
            <div style={{ display:"flex", flexWrap:"wrap", gap:6 }}>
              {result.keywords?.map((k,i) => (
                <span key={i} onClick={()=>copy(k,`kw${i}`)} style={{ background:copied===`kw${i}`?B.green:B.sand, color:copied===`kw${i}`?"#fff":B.muted, fontSize:16, borderRadius:20, padding:"4px 11px", cursor:"pointer", transition:"all .2s" }}>{k}</span>
              ))}
            </div>
            <div style={{ fontSize:15, color:B.muted, marginTop:8 }}>Tap any keyword to copy it</div>
          </div>
          <div style={{ ...S.card, marginBottom:12 }}>
            <div style={{ fontWeight:700, color:B.dark, fontSize:15, marginBottom:5 }}>📋 Suggested Board Name</div>
            <div style={{ fontSize:15, color:"#444", background:B.cream, borderRadius:8, padding:"9px 12px" }}>{result.boardName}</div>
          </div>
          <div style={{ background:B.softGreen, borderRadius:12, padding:"12px 16px", fontSize:15, color:B.green }}>
...
JSON only: {"freebieIdeas":[{"name":"...","why":"...","whatToInclude":"..."}],"bestFreebie":"...","pinPromo":{"title":"...","description":"..."},"thankYouEmail":{"subject":"...","body":"..."},"followUpEmail":{"subject":"...","body":"..."},"whyBetter":"..."}`);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <div style={{ background:B.softGreen, borderRadius:12, padding:"12px 16px", marginBottom:20, fontSize:15, color:B.green }}>
      ...
      {plan.freebieIdeas?.map((f,i) => (
        <div key={i} style={{ ...S.card, marginBottom:12, borderLeft:`4px solid ${i===0?B.red:B.sand}` }}>
          <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
            {i===0 && <span style={{ background:B.red, color:"#fff", fontSize:16, fontWeight:700, borderRadius:5, padding:"2px 7px", flexShrink:0 }}>BEST</span>}
            <div>
              <div style={{ fontWeight:700, color:B.dark, fontSize:16, marginBottom:5 }}>{f.name}</div>
              <div style={{ fontSize:15, color:"#555", marginBottom:5 }}>{f.why}</div>
              <div style={{ fontSize:16, color:B.muted, background:B.warm, borderRadius:7, padding:"6px 10px" }}>📄 Include: {f.whatToInclude}</div>
            </div>
          </div>
        </div>
      ))}
      ...
      <div style={{ ...S.card, marginBottom:20 }}>
        <div style={{ fontWeight:700, color:"#E60023", fontSize:15, marginBottom:5 }}>Pin Title</div>
        <div style={{ fontSize:15, color:B.dark, background:B.cream, borderRadius:8, padding:"9px 12px", marginBottom:12 }}>{plan.pinPromo?.title}</div>
        <div style={{ fontWeight:700, color:"#E60023", fontSize:15, marginBottom:5 }}>Pin Description</div>
        <div style={{ fontSize:15, color:B.dark, background:B.cream, borderRadius:8, padding:"9px 12px" }}>{plan.pinPromo?.description}</div>
      </div>
      <SectionTitle>📧 Email #1 — Thank You Email (Instant)</SectionTitle>
      <div style={{ ...S.card, marginBottom:12 }}>
        <div style={{ fontWeight:700, color:B.dark, fontSize:16, marginBottom:4 }}>Subject: {plan.thankYouEmail?.subject}</div>
        <div style={{ fontSize:15, color:"#555", lineHeight:1.6, background:B.cream, borderRadius:8, padding:"10px 12px", whiteSpace:"pre-wrap" }}>{plan.thankYouEmail?.body}</div>
      </div>
      <SectionTitle>📧 Email #2 — Soft Sell (3 Days Later)</SectionTitle>
      <div style={{ ...S.card, marginBottom:12 }}>
        <div style={{ fontWeight:700, color:B.dark, fontSize:16, marginBottom:4 }}>Subject: {plan.followUpEmail?.subject}</div>
        <div style={{ fontSize:15, color:"#555", lineHeight:1.6, background:B.cream, borderRadius:8, padding:"10px 12px", whiteSpace:"pre-wrap" }}>{plan.followUpEmail?.body}</div>
      </div>
...
Analyze this post performance data and give 4 short, actionable insights for CuriousLittleRed. Be specific. Compare to her known best performer (butterfly DIY = 7,878 views). Tell her what to do MORE of and what to stop.\n\n${summary}\n\nJSON only: {"insights":["...","...","...","..."]}`);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <p style={{ color:B.muted, fontSize:16, marginBottom:18 }}>Log your posts and get AI analysis on what's actually working for CuriousLittleRed.</p>
      {posts.length > 0 && (
        <div style={{ display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:10, marginBottom:20 }}>
          {[["Posts Logged", posts.length,"📝"],["Total Views",totalViews.toLocaleString(),"👁️"],["Top Post",topPost?.views.toLocaleString()||"-","🔥"]].map(([l,v,ic])=>(
            <div key={l} style={{ background:B.cream, border:`1px solid ${B.sand}`, borderRadius:12, padding:"12px", textAlign:"left" }}>
              <div style={{ fontSize:20 }}>{ic}</div>
              <div style={{ fontSize:24, fontWeight:700, color:B.red }}>{v}</div>
              <div style={{ fontSize:15, color:B.muted }}>{l}</div>
            </div>
          ))}
        </div>
      )}
      ...
      {insights.length > 0 && (
        <div style={{ ...S.card, background:B.softGreen, borderColor:B.green, marginBottom:16 }}>
          ...
          {insights.map((ins,i)=><div key={i} style={{ fontSize:15, color:B.dark, marginBottom:7 }}>• {ins}</div>)}
        </div>
      )}
      {posts.length === 0 && <div style={{ textAlign:"left", color:B.muted, padding:"40px 0", fontSize:14 }}>No posts yet. Log your first one! 📝</div>}
      {posts.map(p=>(
        <div key={p.id} style={{ ...S.card, display:"flex", justifyContent:"space-between", marginBottom:10 }}>
          <div>
            ...
            <div style={{ fontSize:15, color:B.muted, marginBottom:6 }}>{p.platform} · {p.date}</div>
...
JSON only: {"title":"...","intro":"...","sections":[{"heading":"...","content":"..."}],"printableMention":"...","closing":"...","pinterestSummary":"..."}`, 2000);
  ...
  return (
    <div style={{ padding:"24px 0" }}>
      <p style={{ color:B.muted, fontSize:16, marginBottom:20 }}>Write blog posts that bring Pinterest moms to your site and naturally lead them to your printables.</p>
      ...
      {suggestions.length > 0 && (
        <div style={{ ...S.card, marginBottom:20 }}>
          <div style={{ fontWeight:700, color:B.dark, fontSize:15, marginBottom:12 }}>💡 Topic Ideas — click one to use it</div>
          <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
            {suggestions.map((s,i) => (
              <div key={i} onClick={()=>{ setTopic(s); setSuggestions([]); }} style={{ background:B.cream, border:`1px solid ${B.sand}`, borderRadius:10, padding:"10px 14px", fontSize:15, color:B.dark, cursor:"pointer" }}>
                {s}
              </div>
            ))}
          </div>
      ...
      {post && (
        <div style={{ marginTop:10 }}>
          ...
          <div style={{ ...S.card, marginBottom:12 }}>
            <div style={{ fontWeight:700, color:B.muted, fontSize:15, textTransform:"uppercase", letterSpacing:0.8, marginBottom:8 }}>Intro</div>
            <div style={{ fontSize:16, color:B.dark, lineHeight:1.7 }}>{post.intro}</div>
          </div>

          {post.sections?.map((sec,i) => (
            <div key={i} style={{ ...S.card, marginBottom:12 }}>
              <div style={{ fontWeight:700, color:B.dark, fontSize:17, marginBottom:8 }}>{sec.heading}</div>
              <div style={{ fontSize:16, color:"#444", lineHeight:1.7 }}>{sec.content}</div>
            </div>
          ))}

          <div style={{ ...S.card, marginBottom:12, borderLeft:`4px solid ${B.red}` }}>
            <div style={{ fontWeight:700, color:B.red, fontSize:15, textTransform:"uppercase", letterSpacing:0.8, marginBottom:8 }}>Printable Mention</div>
            <div style={{ fontSize:16, color:B.dark, lineHeight:1.7 }}>{post.printableMention}</div>
          </div>

          <div style={{ ...S.card, marginBottom:12 }}>
            <div style={{ fontWeight:700, color:B.muted, fontSize:15, textTransform:"uppercase", letterSpacing:0.8, marginBottom:8 }}>Closing</div>
            <div style={{ fontSize:16, color:B.dark, lineHeight:1.7 }}>{post.closing}</div>
          </div>

          <div style={{ background:B.softGold, borderRadius:12, padding:"14px 16px" }}>
            <div style={{ fontWeight:700, color:B.gold, fontSize:15, textTransform:"uppercase", letterSpacing:0.8, marginBottom:6 }}>📌 Pinterest Summary</div>
            <div style={{ fontSize:15, color:B.dark, lineHeight:1.6 }}>{post.pinterestSummary}</div>
          </div>
        </div>
      )}
    </div>
      ...
      <div style={{ background:`linear-gradient(135deg, ${B.darkRed} 0%, ${B.red} 100%)`, paddingBottom:0, boxShadow:"0 4px 20px rgba(0,0,0,0.2)" }}>
        <div style={{ maxWidth:740, margin:"0 auto", padding:"18px 20px 0" }}>
          <div style={{ display:"flex", alignItems:"center", gap:12, marginBottom:16 }}>
            ...
            <div>
              <div style={{ color:"#fff", fontWeight:700, fontSize:24, lineHeight:1.1 }}>CuriousLittleRed</div>
              <div style={{ color:"rgba(255,255,255,0.7)", fontSize:15, letterSpacing:0.5 }}>Content Command Center v2 · Built for your 130K Pinterest viewers</div>
            </div>
          </div>
